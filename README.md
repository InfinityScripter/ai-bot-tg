...
Автономный Telegram-бот, который раз в день собирает до `MAX_PER_RUN` новостей
из проверенных RSS-лент, переписывает их активной LLM-моделью и сразу публикует
в блог (`aifirst.us.com`) без ручного подтверждения. Telegram-карточка показывает
ход и итог обработки; при сбое оставляет кнопки для ручного восстановления.
Ссылки и тексты, присланные владельцем вручную, сохраняют preview + approve flow.

<p align="center">
  <img src="docs/news-bot-pipeline.gif" alt="Конвейер новостного бота: RSS → рерайт LLM → автоматическая публикация" width="720">
</p>

Рерайт — не нейтральный пересказ. Промпт работает в голосе Михаила как
инженера-практика и строится вокруг одного главного вывода, которого нет в
заголовке. Формат под чтение с телефона: короткие абзацы, связный текст вместо
списков, честный блок с минусами и ограничениями, вопрос читателю в конце.
Цены и «что бесплатно» называются только когда об этом сказал источник; первое
лицо в авторских черновиках сохраняется. Перед публикацией production-финализатор
разбирает Markdown в AST, удаляет HTML и оставляет только ссылки и картинки из
исходника. Prompt eval ловит шаблонные тексты, новые числа/цитаты и потерю
авторского голоса.
...
│   ├── createAutoPublish.ts # автоматический batch rewrite + publish
...
│   ├── candidateMutations.ts # атомарные переходы (claim/attach/clearAutoPublish/…)
