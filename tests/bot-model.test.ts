...
import { MODEL_CALLBACK } from "../src/consts.js";
import { CallbackKind, ProviderName } from "../src/enums.js";
import {
  statusText,
  encodeModel,
  modelButtons,
...
describe("callback data round-trip", () => {
  ...
  it("encodes and parses a model pick, preserving dotted model ids", () => {
    const data = encodeModel(ProviderName.Glm, "glm-4.7-flash");
    expect(parseCallback(data)).toEqual({
      kind: CallbackKind.Model,
      provider: ProviderName.Glm,
      model: "glm-4.7-flash",
    });
  });
  ...
  it("parses reset and back", () => {
    expect(parseCallback(MODEL_CALLBACK.RESET)).toEqual({ kind: CallbackKind.Reset });
    expect(parseCallback(MODEL_CALLBACK.BACK)).toEqual({ kind: CallbackKind.Back });
  });
  ...
  it("keeps every callback under Telegram 64-byte limit for known providers", () => {
    ...
    const longModel = encodeModel(ProviderName.DeepSeek, "deepseek-v4-flash-preview-extended");
...
describe("button specs", () => {
  it("providerButtons marks providers without a key using 🔑", () => {
    ...
    for (const b of buttons) expect(b.data.startsWith(MODEL_CALLBACK.PROVIDER)).toBe(true);
  });

  it("modelButtons lists models (with price hints) and appends a back button", () => {
    const buttons = modelButtons(ProviderName.Glm, ["glm-4.7-flash", "glm-4.7"]);
    // callback data stays the bare encoded id; the visible text gains a hint.
    expect(buttons.map((b) => b.data)).toEqual([
      encodeModel(ProviderName.Glm, "glm-4.7-flash"),
      encodeModel(ProviderName.Glm, "glm-4.7"),
      MODEL_CALLBACK.BACK,
    ]);
  ...
  it("modelButtons drops a model whose callback data would exceed 64 bytes", () => {
    ...
    const buttons = modelButtons(ProviderName.Glm, ["glm-4.7-flash", tooLong]);
    expect(buttons.map((b) => b.data)).toEqual([
      encodeModel(ProviderName.Glm, "glm-4.7-flash"),
      MODEL_CALLBACK.BACK,
    ]);
...
describe("statusText", () => {
  it("reports env source when no override", () => {
    const text = statusText({ provider: ProviderName.Glm, model: "glm-4.7-flash" }, false);
    expect(text).toContain("GLM / glm-4.7-flash");
    expect(text).toContain("env");
  });

  it("reports override source when overridden", () => {
    const text = statusText({ provider: ProviderName.DeepSeek, model: "deepseek-v4-flash" }, true);
    expect(text).toContain("override");
  });

  it("shows the mock notice when mock is active", () => {
    const text = statusText({ provider: ProviderName.Glm, model: "glm-4.7-flash" }, false, true);
...
describe("mock toggle", () => {
  it("parses mmock_on / mmock_off", () => {
    expect(parseCallback(MODEL_CALLBACK.MOCK_ON)).toEqual({ kind: CallbackKind.MockOn });
    expect(parseCallback(MODEL_CALLBACK.MOCK_OFF)).toEqual({ kind: CallbackKind.MockOff });
  });

  it("mockToggleButton reflects current state and offers the opposite action", () => {
    const on = mockToggleButton(true);
    expect(on.data).toBe(MODEL_CALLBACK.MOCK_OFF); // mock ON → button turns it OFF
    expect(on.text).toMatch(/выключить/i);

    const off = mockToggleButton(false);
    expect(off.data).toBe(MODEL_CALLBACK.MOCK_ON);
    expect(off.text).toMatch(/включить/i);
  });

  it("mock-toggle callback data stays within Telegram 64-byte limit", () => {
    expect(Buffer.byteLength(MODEL_CALLBACK.MOCK_ON, "utf8")).toBeLessThanOrEqual(64);
    expect(Buffer.byteLength(MODEL_CALLBACK.MOCK_OFF, "utf8")).toBeLessThanOrEqual(64);
  });
});
