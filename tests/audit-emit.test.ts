...
import { relevanceActionFor, emitRelevanceDecision, emitRelevanceDecisions } from '../src/audit-emit.js';
...
describe('relevanceActionFor', () => {
  ...
  it('dropped in shadow mode → bot.relevance_shadow_dropped', () => {
    expect(relevanceActionFor(decision({ kept: false }), 'shadow')).toBe('bot.relevance_shadow_dropped');
  });
